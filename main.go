...
func skakeda(w http.ResponseWriter, r *http.Request) {
    db, err := sql.Open("mysql", "skakeda:skakeda_App_2022@tcp(10.35.0.59:3306)/seweb")
                if err != nil {
                        panic(err.Error())
                }
    defer db.Close()

     rows, err := db.Query("SELECT body FROM skakeda_web WHERE id=1")
                if err != nil {
                        panic(err.Error())
                }
    ...
    rows.Scan(&body)
// log.Print(body)
    t , err := template.ParseFiles("skakeda.tpl")
                if err != nil {
                        panic(err.Error())
                }
    t.Execute(w, body)
